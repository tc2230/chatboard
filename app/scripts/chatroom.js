...
document.addEventListener('DOMContentLoaded', () => {
  ...
  sendForm.addEventListener('submit', function (e) {
    ...
    let formData = {
      timestamp: `${current_time.getFullYear()}-${String(
        current_time.getMonth() + 1,
      ).padStart(2, '0')}-${String(current_time.getDate()).padStart(
        2,
        '0',
      )}  ${String(current_time.getHours()).padStart(2, '0')}:${String(
    ...
    for (let i = 0; i < sendForm.childElementCount; i++) {
      ...
      if (child.name !== '') {
        ...
        if (val === '' || !val) {
          // submit string is invalid if empty after trimmed. (for both name and msg)
          valid = false;
